...
{
	m_CalibrationMode = GetConfig () -> GetProfileInt ( "Argyll", "DeviceCalibrationMode", 999 );
	if ( m_CalibrationMode > 4 )
	{
		// No new format calibration mode recorded: try to load old format
		m_CalibrationMode = GetConfig () -> GetProfileInt ( "Argyll", "CalibrationMode", 1 );
		...
		GetConfig () -> WriteProfileInt ( "Argyll", "DeviceCalibrationMode", m_CalibrationMode );
	}

	m_bAmbientLight = GetConfig () -> GetProfileInt ( "Argyll", "AmbientLight", 0 );
	m_debugMode = FALSE;
	m_bAverageReads = GetConfig () -> GetProfileInt ( "Argyll", "AverageReads", FALSE );
	m_NbMinutesCalibrationValid = GetConfig () -> GetProfileInt ( "Argyll", "MinutesCalibrationValid", 0 );
	m_MinRequestedY = GetConfig () -> GetProfileDouble ( "Argyll", "MinRequestedY", 10.0 );
	m_MaxReadLoops = GetConfig () -> GetProfileInt ( "Argyll", "MaxReadLoops", 8 );
...
void CArgyllSensor::Copy(CSensor * p)
{
	...
	m_CalibrationMode = ((CArgyllSensor*)p)->m_CalibrationMode;
	m_bAmbientLight = ((CArgyllSensor*)p)->m_bAmbientLight;
	m_debugMode = ((CArgyllSensor*)p)->m_debugMode;	
	m_bAverageReads = ((CArgyllSensor*)p)->m_bAverageReads;
	m_NbMinutesCalibrationValid = ((CArgyllSensor*)p)->m_NbMinutesCalibrationValid;
}
...
void CArgyllSensor::SetPropertiesSheetValues()
{
	...
	m_ArgyllSensorPropertiesPage.m_CalibrationMode=m_CalibrationMode;
	m_ArgyllSensorPropertiesPage.m_bAmbientLight=m_bAmbientLight;
	m_ArgyllSensorPropertiesPage.m_debugMode=m_debugMode;
	m_ArgyllSensorPropertiesPage.m_bAverageReads=m_bAverageReads;
	m_ArgyllSensorPropertiesPage.m_NbMinutesCalibrationValid=m_NbMinutesCalibrationValid;
}
...
void CArgyllSensor::GetPropertiesSheetValues()
{
	...
	if( m_CalibrationMode != m_ArgyllSensorPropertiesPage.m_CalibrationMode ) {
		m_CalibrationMode=m_ArgyllSensorPropertiesPage.m_CalibrationMode;
	...
	if( m_bAverageReads != m_ArgyllSensorPropertiesPage.m_bAverageReads ) {
		m_bAverageReads=m_ArgyllSensorPropertiesPage.m_bAverageReads;
		SetModifiedFlag(TRUE);

		GetConfig () -> WriteProfileInt ( "Argyll", "AverageReads", m_bAverageReads );
	}

	if( m_NbMinutesCalibrationValid != m_ArgyllSensorPropertiesPage.m_NbMinutesCalibrationValid ) {
		m_NbMinutesCalibrationValid=m_ArgyllSensorPropertiesPage.m_NbMinutesCalibrationValid;
		SetModifiedFlag(TRUE);

		GetConfig () -> WriteProfileInt ( "Argyll", "MinutesCalibrationValid", m_NbMinutesCalibrationValid );
	}

	if( m_bAmbientLight != m_ArgyllSensorPropertiesPage.m_bAmbientLight ) {
		m_bAmbientLight=m_ArgyllSensorPropertiesPage.m_bAmbientLight;
		SetModifiedFlag(TRUE);

		GetConfig () -> WriteProfileInt ( "Argyll", "AmbientLight", m_bAmbientLight );
	}
}

void CArgyllSensor::CreateArgyllPipe ()
{
	DWORD		dw;
	char		szBuf [ 512 ];
	STARTUPINFO	si;
...
void CArgyllSensor::CloseArgyllPipe ()
{
	if ( m_hPipe )
	{
		DWORD	dw;
		...
		if ( bOk )
		{
			...
			if ( ! bContinue )
			{
				...
				if ( bSpectrumOk )
				{
					...
					ArgyllColor.SetSpectrum ( CSpectrum ( SPECTRUM_BANDS, SPECTRUM_WAVELENGTH_MIN, SPECTRUM_WAVELENGTH_MAX, SPECTRUM_WAVELENGTH_BANDWIDTH, FullSpectrum ) );
				}
