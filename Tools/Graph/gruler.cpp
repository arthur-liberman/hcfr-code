...
// GG mod: use double width for vertical ruler -> shift twice
//	int MinShift = ruler_width;
	int MinShift = ruler_width * 2;
// End of GG mod
	int MaxShift=0;
			// Filling Rectangle
...
{
	int ruler_width = Width(dc_to_draw);
// GG mod: use double width for vertical ruler, not horizontal one !
//	int MinShift = ruler_width;
	int MinShift = ruler_width / 4;
// End of GG mod
	int MaxShift=0;
...
//			    dc_to_draw->TextOut(rect_to_draw.right-TM.tmHeight,CurrentY+(Sz.cx >> 1),s);
		        dc_to_draw->TextOut( ( (rect_to_draw.left+rect_to_draw.right) >> 1 ) - (Sz.cx >> 1) ,CurrentY-(Sz.cy >> 1),s);
...
{
// GG mod: use double width for vertical ruler
//    return ConvertLogPixToRealPix(dest_dc, FRulerWidth, TRUE);
    return ConvertLogPixToRealPix(dest_dc, FRulerWidth * 2, TRUE);
// End of GG mod
}
