...
- (BOOL)trackMouse:(NSEvent *)theEvent inRect:(NSRect)cellFrame ofView:(NSView *)controlView untilMouseUp:(BOOL)untilMouseUp
{
	...
	while (!done)
	{
		...
		if ([self menu])
    {
      NSPoint pointInView = [controlView convertPoint:currentPoint fromView:nil];

      // if the clic happend on the arrow (lower right corner),
      // display the menu immediatly. Else, wait a bit.
      if (pointInView.x > cellFrame.size.width-8 && pointInView.y > cellFrame.size.height-8)
        endDate = [NSDate date];
      else
...
-(void) drawRect:(NSRect)rect
{
  ...
  [path moveToPoint:NSMakePoint(24.0,24.0)];
  [path lineToPoint:NSMakePoint(32.0,24.0)];
  [path lineToPoint:NSMakePoint(28.0,32.0)];
  [path closePath];
