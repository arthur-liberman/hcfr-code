...
void CColorTempGrapher::UpdateGraph ( CDataSetDoc * pDoc )
{
	...
	for (int i=1; i<size; i++)
	{
		...
		if(colorTemp > 1500 && colorTemp < 12000)
			m_graphCtrl.AddPoint(m_ColorTempGraphID, ArrayIndexToGrayLevel ( i, size, GetConfig () -> m_bUseRoundDown, GetConfig () -> m_bUse10bit ), colorTemp);
		...
		if ( m_showDataRef && pDataRef !=NULL && pDataRef != pDoc )
		{
			...
			if(colorTemp > 1500 && colorTemp < 12000)
				m_graphCtrl.AddPoint(m_ColorTempDataRefGraphID, ArrayIndexToGrayLevel ( i, size, GetConfig () -> m_bUseRoundDown, GetConfig () -> m_bUse10bit ), colorTemp);
		}
	}
}
