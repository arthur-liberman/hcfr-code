...
/*
			if ( GetConfig()->m_bUseReferenceGamma )
			{
				if( GetConfig()->m_colorStandard == sRGB )
					val=voltage_to_intensity_srgb((double)(i + 101 - size)/100.0);
				else
					val=voltage_to_intensity_rec709((double)(i + 101 - size)/100.0);
			}
*/
...
void CNearWhiteGrapher::AddPointtoLumGraph(int ColorSpace,int ColorIndex,int Size,int PointIndex,CDataSetDoc *pDataSet,long GraphID,long LogGraphID = -1)
{
	...
	if (ColorSpace == 0) 
	{
		max=pDataSet->GetMeasure()->GetNearWhite(Size-1).GetRGBValue()[ColorIndex];
		colorlevel=pDataSet->GetMeasure()->GetNearWhite(PointIndex).GetRGBValue()[ColorIndex];
	}
	else if (ColorSpace == 1)
	{
		max=pDataSet->GetMeasure()->GetNearWhite(Size-1).GetLuxOrLumaValue();
		colorlevel=pDataSet->GetMeasure()->GetNearWhite(PointIndex).GetLuxOrLumaValue();
	}
	else 
	{
...
void CNearWhiteHistoView::OnUpdate(CView* pSender, LPARAM lHint, CObject* pHint) 
{
	...
	if ( lHint == UPD_EVERYTHING || lHint == UPD_NEARWHITE || lHint == UPD_DATAREFDOC || lHint == UPD_REFERENCEDATA || lHint == UPD_ARRAYSIZES || lHint == UPD_GENERALREFERENCES || lHint == UPD_SENSORCONFIG )
	{
